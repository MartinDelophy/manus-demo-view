        ...
        switch (data.type) {
          case "STEP_UPDATE": {
            ...
            if (stepsRef.current.some((step) => step.id === updatedStep.id)) {
              newSteps = stepsRef.current.map((step) => (step.id === updatedStep.id ? updatedStep : step))
            } else {
              // 否则添加新步骤
            ...
            if (updatedStep.status === "running") {
              console.log("watch", newSteps, updatedStep.id)
              const newIndex = newSteps.findIndex((step) => step.id === updatedStep.id)
